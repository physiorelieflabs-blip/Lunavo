...
**How to apply:** Use the server-only connector proxy for Whop API calls, keep provider IDs in non-secret configuration, match verified payments to the local pending record by checkout identity, amount, currency, and successful provider status, and fail closed for unsupported currencies or lifecycle states. A hosted redirect alone is never payment evidence. The connector does not require a Whop API key or webhook secret for checkout, verification, or provider-backed refunds; without webhooks, renewals, disputes, and asynchronous refund notifications are not automatic.