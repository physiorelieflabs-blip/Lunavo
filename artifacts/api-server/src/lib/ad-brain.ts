export type AdBrainInput = {
  ...
  audience?: string | null; goal?: string | null; offer?: string | null; customPrompt?: string | null;
};
export type AdBrainPlan = { hook:string; valueProp:string; proof:string; cta:string; caption:string; hashtags:string[]; script:Array<{seconds:number;text:string;scene:'hook'|'product'|'proof'|'cta'}>; score:number; reasoning:string[]; source:'local_fallback'|'model'; };
...
export function planAd(input:AdBrainInput):AdBrainPlan {
 ...
 const price=typeof input.price==='number'&&Number.isFinite(input.price)?currency+' '+input.price.toFixed(2):'';
 const goal=clean(input.goal,'sales').toLowerCase(), offer=clean(input.offer), customPrompt=clean(input.customPrompt);
 const words=desc.split(' ').filter(Boolean);
 const benefit=customPrompt|| (words.length>=6?words.slice(0,12).join(' '):'Discover '+title.toLowerCase()+' with a clear value proposition');
 ...
 const hook=customPrompt?customPrompt.slice(0,180):(hooks[Math.abs(title.length)%hooks.length]!);
...
export async function planAdWithBrain(input: AdBrainInput): Promise<AdBrainPlan> {
  ...
  try {
    ...
    const script=rawScript.flatMap((item:unknown)=>{if(!item||typeof item!=='object')return [];const row=item as Record<string,unknown>;const sceneValue=String(row.scene??'');if(!(['hook','product','proof','cta'] as const).includes(sceneValue as never))return [];const seconds=Number(row.seconds);const text=str(row.text,240);return Number.isFinite(seconds)&&seconds>0&&text?[{seconds:Math.min(20,Math.max(2,Math.round(seconds))),text,scene:sceneValue as 'hook'|'product'|'proof'|'cta'}]:[];});
