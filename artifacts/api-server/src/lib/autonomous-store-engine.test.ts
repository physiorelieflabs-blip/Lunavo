...
describe("autonomous store engine", () => {
  it("keeps pricing unavailable when authoritative market prices are missing", () => {
    const result = buildAutonomousOpportunity({
      externalKey: "supplier-1", productName: "Example product", currency: "USD", landedCostMinor: 1000,
      marketLowMinor: null, marketHighMinor: null, demand: 90, lowCompetition: 95, growth: 80, supplier: 90, marketFit: 90, risk: 5,
    });
  ...
  it("ranks high-demand and low-competition opportunities first", () => {
    const base = { currency: "USD", landedCostMinor: 1000, marketLowMinor: 1800, marketHighMinor: 2200, growth: 80, supplier: 85, marketFit: 90, risk: 5 } as const;
  ...
  it("does not silently cap a configured count at the old 100-slot limit", () => {
    expect(buildDailyAdSlots(101, 8, 22)).toHaveLength(101);
  });
});
