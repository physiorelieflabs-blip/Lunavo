import { calculatePriceCeiling, normalizeDailyAdCount } from "./autonomous-store-policy";
...
/** Build evenly distributed local-time ad slots without silently truncating the merchant's configured daily count. */
export function buildDailyAdSlots(dailyCount:number,startHour=8,endHour=22){
 const count=normalizeDailyAdCount(dailyCount);
 if(!count)return[] as Date[];
 const start=Math.max(0,Math.min(23,Math.trunc(startHour))),end=Math.max(start+1,Math.min(24,Math.trunc(endHour))),span=(end-start)*60*60*1000;
 return Array.from({length:count},(_,i)=>{const d=new Date();d.setHours(start,0,0,0);d.setTime(d.getTime()+Math.floor((i+.5)*span/count));return d;});
}
