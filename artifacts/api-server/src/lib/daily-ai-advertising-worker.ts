...
let running = false;
function idempotency(merchantId: number, slot: number, channel: string, productId: number, date: string) { return createHash("sha256").update(`daily-ai-ad:${merchantId}:${date}:${slot}:${channel}:${productId}`).digest("hex"); }
...
/** Creates daily plans in each merchant's local calendar day. Plans are not external publications. */
export async function runDailyAiAdvertisingPlanner() {
  ...
  try{
    ...
    for(const merchant of merchants.rows as Array<{id:number;daily_ad_limit:number;enabled:boolean;channels:unknown;timezone:string}>){
      if(!merchant.enabled)continue; const limit=Math.max(0,Math.min(100,Number(merchant.daily_ad_limit??3))); const channels=Array.isArray(merchant.channels)?merchant.channels.filter((v):v is string=>typeof v==="string"):[]; if(!limit||!channels.length)continue;
      const today=merchantToday(merchant.timezone);
      try{await db.transaction(async tx=>{
        const existing=await tx.execute(sql`SELECT COUNT(*)::int AS count FROM ai_daily_ad_plans WHERE merchant_id=${merchant.id} AND plan_date=${today.date}`); if(Number((existing.rows[0] as {count?:number}|undefined)?.count??0)>=limit)return;
        const products=await tx.execute(sql`SELECT id,title FROM supplier_products WHERE merchant_id=${merchant.id} AND status='active' AND visibility='active' ORDER BY updated_at DESC LIMIT 100`); const rows=products.rows as Array<{id:number;title:string}>; if(!rows.length)return;
        for(let slot=1;slot<=limit;slot++){const product=rows[(slot-1)%rows.length],channel=channels[(slot-1)%channels.length];if(!product||!channel)continue;const key=idempotency(merchant.id,slot,channel,Number(product.id),today.date);const scheduledAt=new Date(scheduledSlot(today.date,merchant.timezone,slot-1,limit));const inserted=await tx.execute(sql`INSERT INTO ai_daily_ad_plans (merchant_id,plan_date,slot_index,scheduled_at,product_id,channel,status,reason) VALUES (${merchant.id},${today.date},${slot},${scheduledAt.toISOString()},${product.id},${channel},'planned',${`Daily AI slot ${slot}: ${product.title}`}) ON CONFLICT (merchant_id,plan_date,slot_index) DO NOTHING RETURNING id`);if(!inserted.rows.length)continue;await tx.execute(sql`INSERT INTO merchant_automation_audit (merchant_id,action_type,status,entity_type,entity_id,idempotency_key,reason,metadata) VALUES (${merchant.id},'daily_ad_plan','planned','ai_daily_ad_plan',${String((inserted.rows[0] as {id?:number}).id??'')},${key},'Server daily AI advertising planner',${JSON.stringify({slot,channel,productId:product.id,timezone:merchant.timezone,localDate:today.date})}::jsonb) ON CONFLICT (idempotency_key) DO NOTHING`);}
