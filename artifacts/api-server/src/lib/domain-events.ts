...
export async function emitDomainEvent(executor: EventExecutor, input: EmitDomainEventInput) {
  ...
  const [existing] = await executor.select().from(domainEventsTable).where(and(
    eq(domainEventsTable.merchantId, input.merchantId), eq(domainEventsTable.idempotencyKey, input.idempotencyKey),
  )).limit(1);
...
function notificationFor(event: typeof domainEventsTable.$inferSelect) {
  ...
  const labels: Partial<Record<DomainEventType, [string, string, string]>> = {
    ...
    "inventory.adjusted": ["Inventory adjusted", "An inventory movement was recorded.", "info"], "inventory.reserved": ["Inventory reserved", "Stock is held pending payment verification.", "info"],
    "inventory.released": ["Inventory reservation released", "A pending stock hold was released.", "warning"], "inventory.committed": ["Inventory committed", "Reserved stock was committed after verified payment.", "success"],
