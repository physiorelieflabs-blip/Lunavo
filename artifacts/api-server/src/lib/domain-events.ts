...
/** Only facts emitted by authoritative commerce mutations belong here. */
export const domainEventTypes = [
  ...
  "marketplace.listing_reviewed", "marketplace.fee_reviewed", "advertising.payment_submitted", "advertising.payment_confirmed", "advertising.payment_reviewed",
...
export async function emitDomainEvent(executor: EventExecutor, input: EmitDomainEventInput) {
  if (!Number.isInteger(input.merchantId) || input.merchantId <= 0) throw new Error("Domain events require a valid tenant merchantId");
  if (!input.idempotencyKey.trim() || !input.aggregateType.trim() || !String(input.aggregateId).trim()) throw new Error("Domain events require aggregate and idempotency identifiers");
  const id = randomUUID();
  const [inserted] = await executor.insert(domainEventsTable).values({
    id, merchantId: input.merchantId, eventType: input.eventType, payloadVersion: input.payloadVersion ?? 1,
    aggregateType: input.aggregateType, aggregateId: String(input.aggregateId), actorType: input.actorType,
    actorId: input.actorId ?? null, source: input.source, idempotencyKey: input.idempotencyKey,
    payload: input.payload, before: input.before ?? null, after: input.after ?? null, context: input.context ?? {},
    correlationId: input.correlationId ?? null, causationId: input.causationId ?? null,
  }).onConflictDoNothing().returning();
  if (inserted) return inserted;
  const [existing] = await executor.select().from(domainEventsTable).where(and(eq(domainEventsTable.merchantId, input.merchantId), eq(domainEventsTable.idempotencyKey, input.idempotencyKey))).limit(1);
  if (!existing) throw new Error("Domain event idempotency conflict could not be resolved");
  return existing;
}
export function retryDelayMs(attempt: number): number { return Math.min(60 * 60_000, 1_000 * 2 ** Math.max(0, attempt - 1)); }
export function nextEventState(attempt: number, maxAttempts = 8): "retry" | "dead_letter" { return attempt >= maxAttempts ? "dead_letter" : "retry"; }
...
function notificationFor(event: typeof domainEventsTable.$inferSelect) {
  ...
  const labels: Partial<Record<DomainEventType, [string, string, string]>> = {
    ...
    "invoice.sent": ["Invoice sent", "An invoice is now available to the customer.", "info"], "invoice.payment_submitted": ["Invoice payment submitted", "Customer payment evidence is awaiting review.", "warning"], "invoice.payment_verified": ["Invoice payment verified", "Invoice payment was verified and posted.", "success"],
    "inventory.adjusted": ["Inventory adjusted", "An inventory movement was recorded.", "info"], "inventory.reserved": ["Inventory reserved", "Stock is held pending payment verification.", "info"], "inventory.released": ["Inventory reservation released", "A pending stock hold was released.", "warning"], "inventory.committed": ["Inventory committed", "Reserved stock was committed after verified payment.", "success"],
    "location.created": ["Location added", "A new merchant location was created.", "info"], "location.updated": ["Location updated", "A merchant location was updated.", "info"], "location.disabled": ["Location disabled", "A merchant location was disabled.", "warning"],
    "invitation.created": ["Team invitation created", "A staff invitation is ready to share.", "info"], "invitation.revoked": ["Team invitation revoked", "A staff invitation was revoked.", "warning"], "invitation.accepted": ["Team invitation accepted", "A staff member joined the workspace.", "success"],
...
async function projectNotification(tx: Pick<typeof db, "insert">, event: typeof domainEventsTable.$inferSelect) {
  const detail = notificationFor(event); if (!detail) return;
  await tx.insert(notificationsTable).values({ id: randomUUID(), merchantId: event.merchantId, eventId: event.id, recipientType: "merchant", actorType: event.actorType, actorId: event.actorId, entityType: event.aggregateType, entityId: event.aggregateId, title: detail.title, body: detail.body, severity: detail.severity, deepLink: detail.deepLink, actionLabel: detail.deepLink ? "Open record" : null, dedupeKey: `event:${event.id}:notification` }).onConflictDoNothing();
}
async function claimNextDomainEvent() {
  return db.transaction(async (tx) => {
    await tx.update(domainEventsTable).set({ status: "retry" }).where(and(eq(domainEventsTable.status, "processing"), lte(domainEventsTable.nextAttemptAt, new Date())));
    const result = await tx.execute(sql<typeof domainEventsTable.$inferSelect>`SELECT * FROM domain_events WHERE status IN ('pending', 'retry') AND next_attempt_at <= now() ORDER BY occurred_at ASC FOR UPDATE SKIP LOCKED LIMIT 1`);
    const event = result.rows[0] as typeof domainEventsTable.$inferSelect | undefined; if (!event) return null;
    const [claimed] = await tx.update(domainEventsTable).set({ status: "processing", attempts: event.attempts + 1, nextAttemptAt: new Date(Date.now() + 5 * 60_000) }).where(and(eq(domainEventsTable.id, event.id), eq(domainEventsTable.status, event.status))).returning();
    return claimed ?? null;
  });
}
async function processClaimedDomainEvent(event: typeof domainEventsTable.$inferSelect) {
  try {
    ...
    const safe = safeProjectionError(error); const state = nextEventState(event.attempts);
    await db.transaction(async (tx) => { await tx.update(domainEventsTable).set({ status: state, lastError: `${safe.code}: ${safe.message}`, nextAttemptAt: new Date(Date.now() + retryDelayMs(event.attempts)) }).where(and(eq(domainEventsTable.id, event.id), eq(domainEventsTable.status, "processing"))); });
