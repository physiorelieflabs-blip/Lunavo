...
function payloadData(payload: Payload): Payload {
...
function eventIdFrom(req: Request, payload: Payload, transactionId: string, eventType: string): string | null {
  ...
  const state = JSON.stringify({
    transactionId,
    eventType,
    status: payloadData(payload).status ?? null,
    refundStatus: payloadData(payload).refund_status ?? null,
    disputeStatus: payloadData(payload).dispute_status ?? null,
  });
  return `derived:${createHash("sha256").update(state).digest("hex")}`;
}
...
router.post("/webhooks/flutterwave", async (req, res): Promise<void> => {
  ...
  if (!eventId) {
    res.status(400).json({ error: "Flutterwave webhook event cannot be identified" });
    return;
  }
  ...
  try {
    ...
    if (!providerTransactionId) {
      await db.update(paymentWebhookEventsTable).set({
        ...
        error: "Missing provider transaction id",
    ...
    // The webhook payload is a signal only. The provider transaction is re-queried
    // and only that verified response is allowed to change the ledger.
    ...
    const outcome = await processVerifiedFlutterwaveTransaction(
      verified as unknown as Payload,
      eventId,
      payload,
    );
    await db.update(paymentWebhookEventsTable).set({
