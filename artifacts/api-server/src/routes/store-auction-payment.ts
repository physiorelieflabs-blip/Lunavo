...
router.post("/merchant/store-auctions/:id/acquire", async (req, res, next) => {
  try {
    ...
    const result = await db.transaction(async tx => {
      const auctionResult = await tx.execute(sql`SELECT id,storefront_id,status,payment_status,payment_reference FROM store_auction_listings WHERE id=${auctionId} FOR UPDATE`);
      ...
      if (String(auction.status) === "transferred" || String(auction.payment_status) === "verified") throw Object.assign(new Error("Store has already been paid and transferred"), { status: 409 });
      ...
      const bidResult = await tx.execute(sql`SELECT id,bidder_merchant_id,amount,currency,risk_status FROM store_auction_bids WHERE id=${bidId} AND auction_id=${auctionId} FOR UPDATE`);
      ...
      if (Number((winner.rows[0] as { id?: number } | undefined)?.id) !== bidId) throw Object.assign(new Error("Only the current highest accepted bid can acquire the store"), { status: 409 });
      ...
      const txRef = String(auction.payment_reference ?? "").trim() || `LUNAVO-STORE-AUC-${auctionId}-${bidId}-${randomUUID()}`;
      ...
      const store = await tx.execute(sql`SELECT name FROM merchant_storefronts WHERE id=${auction.storefront_id} LIMIT 1`);
      return { auctionId, bidId, amount: Number(bid.amount), currency: String(bid.currency).toUpperCase(), email, txRef, storeName: String((store.rows[0] as any)?.name ?? "Lunavo Store") };
    });
    const checkout = await initializeFlutterwavePayment({ txRef: result.txRef, amount: result.amount, currency: result.currency, redirectUrl: redirectUrl(), customer: { email: result.email, name: result.storeName }, title: `Lunavo Store Acquisition — ${result.storeName}`, paymentOptions: "card,banktransfer,ussd,mobilemoney", meta: { store_auction_id: result.auctionId, store_auction_bid_id: result.bidId, payment_kind: "store_auction" } });
    res.status(201).json({ paymentMethod: "lunavo_store_acquisition_flutterwave", status: "payment_pending", amount: result.amount, currency: result.currency, txRef: checkout.txRef, checkoutUrl: checkout.link, ownershipTransfer: "Automatic after server-side Flutterwave verification" });
