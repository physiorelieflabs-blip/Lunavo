...
const ALLOWED_SECTION_TYPES = new Set([
  "hero", "products", "categories", "text", "image", "gallery", "features", "testimonials",
  "faq", "newsletter", "cta", "announcement", "video", "spacer",
]);
...
function validSections(value: unknown): value is Array<Record<string, unknown>> {
  ...
  return value.every((section) => {
    ...
    return typeof id === "string" && /^[a-zA-Z0-9_-]{1,80}$/.test(id)
      && typeof type === "string" && ALLOWED_SECTION_TYPES.has(type);
  });
}

function canonicalPayload(theme: unknown, sections: unknown) {
  return JSON.stringify({ theme, sections });
}
...
async function merchantContext(req: Request, res: Response) {
  ...
  catch { res.status(403).json({ error: "Store builder permission required" }); return null; }
  return { userId, merchantId: merchant.id };
}

async function ownedStore(storefrontId: string, merchantId: number) {
  return (await db.select().from(merchantStorefrontsTable).where(and(
    eq(merchantStorefrontsTable.id, storefrontId),
    eq(merchantStorefrontsTable.merchantId, merchantId),
  )).limit(1))[0] ?? null;
}

router.get("/storefront-builder/:id/draft", async (req, res): Promise<void> => {
  const ctx = await merchantContext(req, res); if (!ctx) return;
  const store = await ownedStore(req.params.id, ctx.merchantId);
...
router.put("/storefront-builder/:id/draft", async (req, res): Promise<void> => {
  ...
  if (theme === undefined || !validSections(sections)) {
    res.status(400).json({ error: "Invalid storefront theme or sections" }); return;
  }
  if (jsonBytes(theme) > MAX_THEME_BYTES || jsonBytes(sections) > MAX_SECTIONS_BYTES) {
    res.status(413).json({ error: "Storefront content is too large" }); return;
  }
  const [updated] = await db.update(merchantStorefrontsTable).set({ theme, sections, updatedAt: new Date() })
    .where(and(eq(merchantStorefrontsTable.id, store.id), eq(merchantStorefrontsTable.merchantId, ctx.merchantId))).returning();
...
router.post("/storefront-builder/:id/publish", async (req, res): Promise<void> => {
  ...
  const store = await ownedStore(req.params.id, ctx.merchantId);
  if (!store) { res.status(404).json({ error: "Storefront not found" }); return; }
  if (!validSections(store.sections) || jsonBytes(store.theme) > MAX_THEME_BYTES || jsonBytes(store.sections) > MAX_SECTIONS_BYTES) {
    res.status(409).json({ error: "Draft contains invalid or oversized storefront content" }); return;
  }
  const latest = (await db.select({ version: storefrontPublicationSnapshotsTable.version })
    .from(storefrontPublicationSnapshotsTable)
    .where(and(eq(storefrontPublicationSnapshotsTable.storefrontId, store.id), eq(storefrontPublicationSnapshotsTable.merchantId, ctx.merchantId)))
  ...
  const result = await db.transaction(async (tx) => {
    const [snapshot] = await tx.insert(storefrontPublicationSnapshotsTable).values({
      storefrontId: store.id, merchantId: ctx.merchantId, version,
      theme: store.theme, sections: store.sections, contentHash,
      publishedByClerkUserId: ctx.userId,
    }).returning();
    const [updated] = await tx.update(merchantStorefrontsTable).set({ published: true, updatedAt: new Date() })
      .where(and(eq(merchantStorefrontsTable.id, store.id), eq(merchantStorefrontsTable.merchantId, ctx.merchantId))).returning();
...
router.post("/storefront-builder/:id/unpublish", async (req, res): Promise<void> => {
  ...
  const store = await ownedStore(req.params.id, ctx.merchantId);
  if (!store) { res.status(404).json({ error: "Storefront not found" }); return; }
  const [updated] = await db.update(merchantStorefrontsTable).set({ published: false, updatedAt: new Date() })
    .where(and(eq(merchantStorefrontsTable.id, store.id), eq(merchantStorefrontsTable.merchantId, ctx.merchantId))).returning();
...
router.get("/public/storefront/by-host", async (req, res): Promise<void> => {
  ...
  if (!hostname || hostname.length > 253 || hostname.includes("/") || hostname.includes("@")) {
    res.status(400).json({ error: "Invalid hostname" }); return;
  }
  const domain = (await db.select({ storefrontId: merchantStorefrontDomainsTable.storefrontId })
    .from(merchantStorefrontDomainsTable)
    .where(and(eq(merchantStorefrontDomainsTable.hostname, hostname), eq(merchantStorefrontDomainsTable.status, "verified")))
    .limit(1))[0];
  if (!domain?.storefrontId) { res.status(404).json({ error: "Storefront not found" }); return; }
  const store = (await db.select({
    id: merchantStorefrontsTable.id, name: merchantStorefrontsTable.name, slug: merchantStorefrontsTable.slug,
    publicKey: merchantStorefrontsTable.publicKey, theme: merchantStorefrontsTable.theme,
    sections: merchantStorefrontsTable.sections, published: merchantStorefrontsTable.published,
  }).from(merchantStorefrontsTable).where(and(eq(merchantStorefrontsTable.id, domain.storefrontId), eq(merchantStorefrontsTable.published, true))).limit(1))[0];
