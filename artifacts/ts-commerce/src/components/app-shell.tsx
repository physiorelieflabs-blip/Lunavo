...
import { BarChart3, Building2, CreditCard, LayoutDashboard, LogOut, Menu, Settings2, ShieldCheck, Users, X } from 'lucide-react';
...
export function AppShell({ children, admin = false }: { children: ReactNode; admin?: boolean }) {
  ...
  return <div className="noise min-h-[100dvh] bg-[#f1eee7] text-[#182333]">
    ...
    <main className="min-h-[100dvh] md:pl-[252px]"><header className="sticky top-0 z-20 flex h-[70px] items-center justify-between border-b border-[#ddd7cb] bg-[#f1eee7]/95 px-5 backdrop-blur md:px-10"><button className="grid h-9 w-9 place-items-center rounded-lg hover:bg-[#e5e0d6] md:hidden" onClick={() => setOpen(true)} data-testid="button-open-menu"><Menu className="h-5 w-5" /></button><div className="hidden items-center gap-2 text-xs text-[#697687] md:flex"><span className="h-2 w-2 rounded-full bg-[#4d9b7f]" />All systems operational</div><div className="ml-auto flex items-center gap-3"><button className="hidden items-center gap-2 rounded-lg px-2 py-2 text-xs font-bold text-[#697687] hover:bg-[#e5e0d6] sm:flex" data-testid="button-settings"><Settings2 className="h-4 w-4" />Settings</button><div className="grid h-9 w-9 place-items-center rounded-full bg-[#d8e1e3] font-mono text-xs font-bold text-[#315e6c]" data-testid="text-user-avatar">{initials(user?.fullName || user?.primaryEmailAddress?.emailAddress)}</div></div></header><div className="page-enter px-5 py-8 md:px-10 md:py-10">{children}</div></main>
  </div>;
}
