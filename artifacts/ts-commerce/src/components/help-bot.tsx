...
export function HelpBot() {
  ...
  const ask = async (question: string) => {
    ...
    const history: GuideMessage[] = messages
      .filter((message) => message.id !== 1 || message.from === 'user')
      .slice(-8)
    ...
    try {
      const result = await customFetch<{ reply: string }>('/api/ai/guide', {
        ...
        body: JSON.stringify({ message: trimmed, location, history }),
      });
      setMessages((current) => [...current, { id: Date.now() + 1, from: 'ai', text: result.reply }]);
    } catch {
      setMessages((current) => [...current, { id: Date.now() + 1, from: 'ai', text: 'TS Guide AI is temporarily unavailable. Please try again in a moment.' }]);
    } finally {
      setPending(false);
    }
  };
