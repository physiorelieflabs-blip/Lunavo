...
import { getGetAdminOverviewQueryKey, getGetCurrencySettingsQueryKey, getGetDashboardOverviewQueryKey, getGetLinkedBankAccountQueryKey, getGetWithdrawalSecurityQueryKey, getListAdminWithdrawalsQueryKey, useBeginWithdrawalSecuritySetup, useConfirmWithdrawalSecuritySetup, useCreateWithdrawal, useDeleteLinkedBankAccount, useGetAdminOverview, useGetCurrencySettings, useGetLinkedBankAccount, useGetWithdrawalSecurity, useListAdminWithdrawals, useRevealWithdrawalDetails, useReviewWithdrawal, useSaveLinkedBankAccount } from '@workspace/api-client-react';
...
export default function AdminWithdrawals() {
  ...
  const requestAdminWithdrawal = (event: FormEvent) => {
    ...
    createWithdrawal.mutate({ data: { amount: normalizedAmount, currency: overview.data.currency, totpCode, confirmation, idempotencyKey: crypto.randomUUID() } }, {
      onSuccess: () => { setMessage('TS Pay admin withdrawal queued. Funds are reserved while the payout is reviewed.'); setAmount(''); setTotpCode(''); setConfirmation(''); refresh(); },
