...
export default function Billing() {
  ...
  const graceCopy = data.gracePeriodHours === 24 ? '24-hour payment-selection window' : '15-day payment window after method selection';
  ...
  return <AppShell>
    <div className="mx-auto max-w-[1050px]">
      ...
      {locked ? <div className="mt-8"><Notice tone="danger" title="Billing access is locked">The workspace is locked. Retry Pay by bank or choose dashboard earnings to restore access after a verified payment.</Notice></div> : data.daysRemaining <= 7 && <div className="mt-8"><Notice title={`Action needed within ${data.daysRemaining || 1} day${data.daysRemaining === 1 ? '' : 's'}`}>{data.paymentMethod ? `Your selected method has ${data.daysRemaining} day${data.daysRemaining === 1 ? '' : 's'} left. ` : 'Select a payment method within 24 hours. '}Settle the outstanding balance before the window closes.</Notice></div>}
