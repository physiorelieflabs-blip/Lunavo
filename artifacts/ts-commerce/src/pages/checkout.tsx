        ...
        <section className="rounded-2xl border border-[#d9d2c4] bg-[#fbfaf6] p-6 md:p-7">
          ...
          {message && <div className="mt-5"><Notice tone="danger" title="Checkout not completed">{message}</Notice></div>}
           <div className="mt-6 rounded-xl border border-[#bfd6dc] bg-[#eef7f8] p-4">
             ...
             {paymentDestination.isLoading ? <p className="mt-2 text-sm text-[#315e6c]">Checking payment readiness…</p> : paymentDestination.data?.configured ? <p className="mt-2 text-sm leading-5 text-[#315e6c]">After you submit the order, TS Commerce requests a Flutterwave destination generated for this exact amount and payment session. You will see the bank and account details on the receipt.</p> : <p className="mt-2 text-sm leading-5 text-[#9b463d]">This store has not configured payment details, so checkout is temporarily unavailable.</p>}
           </div>
