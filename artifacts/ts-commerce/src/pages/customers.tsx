...
function CustomerNotesPanel({ customer }: { customer: { id: number; name: string; notes: string | null } | null }) {
  ...
  const save = () => {
    setMessage('');
    updateCustomer.mutate({ id: customer.id, data: { notes: notes.trim() || null } }, {
      onSuccess: () => {
        setMessage('Saved');
  ...
  return <section className="fixed bottom-5 left-5 z-50 w-[min(420px,calc(100vw-2.5rem))] rounded-2xl border border-[#d6aa46] bg-[#fffaf0] p-4 shadow-2xl" aria-label="Customer notes editor">
    <div className="flex items-center gap-2"><StickyNote className="h-4 w-4 text-[#85601b]" /><p className="text-sm font-extrabold text-[#765817]">Private notes for {customer.name}</p></div>
    ...
    <div className="mt-3 flex items-center justify-between gap-3"><p className="text-xs text-[#85601b]">{message || `${notes.length}/4000 characters`}</p><Button onClick={save} disabled={updateCustomer.isPending} className="min-h-9 px-3 text-xs"><Save className="h-3.5 w-3.5" />Save notes</Button></div>
  </section>;
}

function downloadCsv(customers: Array<{ name: string; email: string; phone: string | null; notes: string | null; orderCount: number; totalSpent: number }>) {
  const rows = [['Name', 'Email', 'Phone', 'Notes', 'Orders', 'Total spent'], ...customers.map((customer) => [customer.name, customer.email, customer.phone ?? '', customer.notes ?? '', String(customer.orderCount), String(customer.totalSpent)])];
...
export default function Customers() {
  ...
  const filteredCustomers = useMemo(() => {
    ...
    return (customers.data ?? []).filter((customer) => {
      ...
      const matchesSearch = !normalized || `${customer.name} ${customer.email} ${customer.phone ?? ''}`.toLowerCase().includes(normalized);
  ...
  return <AppShell><CustomerNotesPanel customer={selectedCustomer} />
     <div className="mx-auto max-w-[1240px]">
      ...
      <section className="mt-9"><SectionHeading eyebrow="Audience intelligence" title="Customer directory" description="Segments are derived from recorded spend, order frequency, and recency. Historical ledger values are not changed." /><div className="mb-4 flex flex-wrap gap-3"><div className="relative min-w-[240px] flex-1"><Search className="pointer-events-none absolute left-3 top-3 h-4 w-4 text-[#8994a2]" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search name, email, or phone" className={`${inputClass} w-full pl-9`} data-testid="input-customer-search" /></div><select className={inputClass} value={segment} onChange={(event) => setSegment(event.target.value as Segment)} aria-label="Customer segment"><option value="all">All customers</option><option value="repeat">Repeat buyers</option><option value="vip">VIP customers</option><option value="new">New customers</option></select></div>{filteredCustomers.length ? <div className="overflow-hidden rounded-xl border border-[#d9d2c4] bg-[#fbfaf6]"><div className="hidden grid-cols-[1.25fr_1.3fr_.7fr_.8fr_28px] gap-4 border-b border-[#d9d2c4] bg-[#f7f4ed] px-5 py-3 text-[10px] font-extrabold uppercase tracking-[.12em] text-[#697687] md:grid"><span>Customer</span><span>Contact</span><span>Segment</span><span className="text-right">Spent</span><span /></div><div className="divide-y divide-[#ded8cd]">{filteredCustomers.map((customer) => { const customerSegment = segmentFor(customer); return <button key={customer.id} className="grid w-full gap-3 px-5 py-4 text-left transition hover:bg-[#f7f4ed] md:grid-cols-[1.25fr_1.3fr_.7fr_.8fr_28px] md:items-center" onClick={() => setSelectedId(customer.id)} data-testid={`customer-${customer.id}`}><div className="flex items-center gap-3"><div className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-[#e1ebee] text-[#316071]"><UsersRound className="h-4 w-4" /></div><div><p className="text-sm font-extrabold">{customer.name}</p><p className="text-xs text-[#697687]">Added {timeAgo(customer.createdAt)}</p></div></div><div className="text-sm text-[#536174]"><p>{customer.email}</p>{customer.phone && <p className="mt-1 text-xs">{customer.phone}</p>}</div><div><Badge tone={customerSegment.tone}>{customerSegment.label}</Badge><p className="mt-1 text-xs text-[#697687]">{customer.orderCount} order{customer.orderCount === 1 ? '' : 's'}</p></div><div className="text-left font-mono text-sm font-bold md:text-right">{money(customer.totalSpent)}</div><ArrowRight className="hidden h-4 w-4 text-[#8994a2] md:block" /></button>; })}</div></div> : <EmptyState title="No customers match this view" description="Try a different search or segment. Your filters only affect this local view." />}</section>
