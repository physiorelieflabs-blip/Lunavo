...
import { ArrowRight, Download, Mail, Phone, RefreshCw, Search, UsersRound, X } from 'lucide-react';
...
function downloadCsv(customers: Array<{ name: string; email: string; phone: string | null; orderCount: number; totalSpent: number }>) {
  const rows = [['Name', 'Email', 'Phone', 'Orders', 'Total spent'], ...customers.map((customer) => [customer.name, customer.email, customer.phone ?? '', String(customer.orderCount), String(customer.totalSpent)])];
...
export default function Customers() {
  ...
  return <AppShell>
    <div className="mx-auto max-w-[1240px]">
      <div className="flex flex-wrap items-end justify-between gap-5"><div><p className="font-mono text-[10px] uppercase tracking-[.18em] text-[#a2772e]">Customer relationship workspace</p><h1 className="mt-2 text-3xl font-extrabold tracking-[-.06em] md:text-4xl">Know who keeps coming back.</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-[#697687]">Search your audience, understand each buying relationship, and export the exact filtered view for a campaign or report.</p></div><div className="flex flex-wrap gap-2"><Button variant="ghost" onClick={() => { void customers.refetch(); void orders.refetch(); }}><RefreshCw className="h-4 w-4" />Refresh</Button><Button onClick={() => downloadCsv(filteredCustomers)} disabled={!filteredCustomers.length}><Download className="h-4 w-4" />Export CSV</Button></div></div>
