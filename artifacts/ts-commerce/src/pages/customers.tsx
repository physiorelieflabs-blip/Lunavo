...
type SpendByCurrency = { currency: string; totalSpent: number };

function segmentFor(customer: { orderCount: number; createdAt: string; tags: string[] }) {
  if (customer.tags.some((tag) => tag.trim().toLowerCase() === 'vip')) return { label: 'VIP', tone: 'success' as const };
...
function spendLabel(spend: SpendByCurrency[]) {
  return spend.length ? spend.map((item) => money(item.totalSpent, item.currency)).join(' · ') : '—';
}

function totalSpendByCurrency(customers: Array<{ spendByCurrency: SpendByCurrency[] }>) {
  const totals = new Map<string, number>();
  for (const customer of customers) {
    for (const spend of customer.spendByCurrency) {
      totals.set(spend.currency, (totals.get(spend.currency) ?? 0) + spend.totalSpent);
    }
  }
...
function downloadCsv(customers: Array<{ name: string; email: string; phone: string | null; notes: string | null; tags: string[]; marketingConsent: boolean; orderCount: number; spendByCurrency: SpendByCurrency[] }>) {
  const rows = [['Name', 'Email', 'Phone', 'Notes', 'Tags', 'Marketing consent', 'Orders', 'Total spent by currency'], ...customers.map((customer) => [customer.name, customer.email, customer.phone ?? '', customer.notes ?? '', customer.tags.join('|'), customer.marketingConsent ? 'yes' : 'no', String(customer.orderCount), spendLabel(customer.spendByCurrency)])];
...
export default function Customers() {
  ...
  const totalSpent = totalSpendByCurrency(customers.data ?? []);
