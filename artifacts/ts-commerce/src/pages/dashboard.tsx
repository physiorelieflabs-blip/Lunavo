...
export default function Dashboard() {
  ...
  const needsAttention = ['warning', 'suspended', 'past_due'].includes(subscription.status);
   return <AppShell><div className="mx-auto max-w-[1320px]">
      <div className="flex flex-wrap items-end justify-between gap-5"><div><p className="font-mono text-[10px] uppercase tracking-[.18em] text-[#c85d3f]">Merchant overview <span className="text-[#a9a092]">·</span> operating view</p><h1 className="mt-2 text-3xl font-extrabold tracking-[-.06em] md:text-4xl" data-testid="text-store-name">{data.storeName}</h1><p className="mt-1 text-sm text-[#697687]">/{data.storeSlug} <span className="mx-1 text-[#b9b0a3]">·</span> Today’s position, in recorded currency</p></div><div className="flex items-center gap-3"><span className="hidden font-mono text-[10px] uppercase tracking-[.12em] text-[#8994a2] sm:inline" data-testid="text-dashboard-date">{new Date().toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}</span><Badge tone={subscription.status === 'active' ? 'success' : 'warning'}>{subscription.status.replaceAll('_', ' ')}</Badge></div></div>
     ...
     {needsAttention && <div className="mt-7"><Notice tone={subscription.status === 'suspended' ? 'danger' : 'warning'} title={subscription.status === 'suspended' ? 'Your account is suspended' : 'Your subscription needs attention'}>You have {money(subscription.amountDue - subscription.amountPaid, data.currency)} remaining on your platform subscription. <Link href="/billing" className="font-extrabold underline" data-testid="link-notice-billing">Review billing</Link>.</Notice></div>}
      <div className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><MetricCard label="Revenue" value={money(data.revenue, data.currency)} detail={<span className="inline-flex items-center gap-1 text-[#2f6958]"><ArrowUpRight className="h-3.5 w-3.5" />{data.revenueChange}% from last period</span>} icon={<CircleDollarSign className="h-5 w-5" />} /><MetricCard label="Available balance" value={money(data.availableBalance, data.currency)} detail="Ready to transfer" icon={<Banknote className="h-5 w-5" />} accent /><MetricCard label="Orders" value={data.orders.toLocaleString()} detail="Across all channels" icon={<PackageCheck className="h-5 w-5" />} /><MetricCard label="Customers" value={data.customers.toLocaleString()} detail="Unique buyers" icon={<UsersRound className="h-5 w-5" />} /></div>
       <div className="mt-4 grid gap-4 lg:grid-cols-[1.4fr_.8fr]"><section className="rounded-[14px] border border-[#d5cdbd] bg-[#fcfaf5] p-5 shadow-[0_10px_24px_rgba(31,39,48,.04)] md:p-6"><SectionHeading eyebrow="Cash position" title="Revenue movement" description="A view of your recorded gross revenue." action={<Link href="/orders" className="text-xs font-extrabold text-[#b14f36] underline" data-testid="link-record-sale">Record a sale</Link>} />{data.revenueSeries.some((point) => point.amount > 0) ? <div className="flex h-52 items-end gap-2 border-b border-l border-[#d5cdbd] px-3 pb-0 pt-6 md:gap-4">{data.revenueSeries.map((point, index) => { const max = Math.max(...data.revenueSeries.map((item) => item.amount), 1); return <div key={point.label} className="group flex h-full flex-1 flex-col items-center justify-end gap-2"><div className="relative w-full max-w-10 rounded-t-[5px] bg-[#c85d3f] transition-all duration-300 group-hover:bg-[#1f2b38]" style={{ height: `${(point.amount / max) * 88}%` }} title={money(point.amount, data.currency)} data-testid={`bar-revenue-${index}`} /><span className="font-mono text-[10px] text-[#8994a2]">{point.label}</span></div>; })}</div> : <EmptyState title="No revenue recorded yet" description="Record your first paid sale to start building your revenue history." />}</section><section className="rounded-[14px] border border-[#344454] bg-[#1f2b38] p-5 text-[#f8f3e8] shadow-[0_12px_28px_rgba(31,43,56,.14)] md:p-6"><p className="font-mono text-[10px] uppercase tracking-[.16em] text-[#e17d5d]">Settlement view</p><h2 className="mt-3 text-xl font-extrabold">Where funds sit</h2><div className="mt-7 space-y-5"><BalanceRow label="Available" value={money(data.availableBalance, data.currency)} /><BalanceRow label="Pending" value={money(data.pendingBalance, data.currency)} /><BalanceRow label="Reserved for withdrawals" value={money(data.withdrawalReserved, data.currency)} /><BalanceRow label="Held for platform fee" value={money(data.earningsHeldForSubscription, data.currency)} brass /></div><div className="mt-7 border-t border-[#46566a] pt-4 text-xs leading-5 text-[#aab6c2]">Held earnings and withdrawal requests are reserved automatically. Only available funds can be requested.</div></section></div>
