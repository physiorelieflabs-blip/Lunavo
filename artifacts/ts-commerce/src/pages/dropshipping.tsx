...
export default function Dropshipping() {
  ...
  const orderId = new URLSearchParams(location.split('?')[1]?.split('#')[0] ?? '').get('orderId');

  useEffect(() => {
    if (!orderId || !queue.data?.some((item) => String(item.id) === orderId)) return;
    const timer = window.setTimeout(() => {
      document.getElementById(`fulfillment-order-${orderId}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }, 0);
  ...
  return <AppShell>
    <div className="mx-auto max-w-[1180px]">
      ...
      {queue.data.length > 0 && <section className="mt-8 rounded-xl border border-[#d9d2c4] bg-[#fbfaf6] p-5"><SectionHeading eyebrow="Traceability" title="Connected order records" description="Jump from a supplier handoff to the payment, ledger, inventory, and event history for the same order." /><div className="mt-4 flex flex-wrap gap-2">{queue.data.map((item) => <Link key={item.id} href={`/orders/${item.id}`} data-testid={`link-queue-order-context-${item.id}`} className="inline-flex items-center gap-2 rounded-lg border border-[#d9d2c4] bg-[#f7f4ed] px-3 py-2 text-xs font-extrabold text-[#536174] transition hover:border-[#bca26a] hover:text-[#182333]">{item.orderNumber}<Route className="h-3.5 w-3.5" /></Link>)}</div></section>}
