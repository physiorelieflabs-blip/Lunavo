...
export default function MarketplaceManagement() {
  ...
  const [adPayment, setAdPayment] = useState<{
    listingId: number; amount: number; currency: string; paymentUrl: string | null;
    paymentDestination?: { bankName: string; accountName: string; accountNumber: string; amount: number; currency: string; expiresAt: string | null } | null;
    billingId: number;
  } | null>(null);
  const [adPayingId, setAdPayingId] = useState<number | null>(null);
  ...
  const payProductAd = async (listingId: number) => {
    ...
    try {
      ...
      setMessage(error instanceof Error ? error.message : 'The $5 product advertising payment could not be started.');
    } finally {
      setAdPayingId(null);
    }
  };
  ...
  return <AppShell>
    <div className="mx-auto max-w-[1200px]">
      ...
      {adPayment && <div className="fixed inset-0 z-50 grid place-items-center bg-[#182333]/65 p-5">
        <div className="w-full max-w-2xl rounded-2xl border border-[#d9d2c4] bg-[#fbfaf6] p-6 shadow-[0_30px_90px_rgba(0,0,0,.25)]">
          <div className="flex items-start justify-between gap-4"><div><p className="font-mono text-[10px] uppercase tracking-[.16em] text-[#a2772e]">Product advertising payment</p><h2 className="mt-2 text-2xl font-extrabold">Pay to promote this product in General Store.</h2><p className="mt-2 text-sm leading-6 text-[#697687]">This is a separate $5 product advertising fee. The listing becomes publicly discoverable only after the provider payment is verified.</p></div><button type="button" onClick={() => setAdPayment(null)} className="rounded-lg p-2 text-[#697687] hover:bg-[#f0ece3]" aria-label="Close"><X className="h-5 w-5" /></button></div>
          <div className="mt-6 grid gap-3 sm:grid-cols-3"><div className="rounded-xl border border-[#d9d2c4] bg-[#f7f4ed] p-4"><p className="text-[10px] uppercase text-[#7d8792]">Amount</p><p className="mt-2 font-mono text-xl font-extrabold">{money(adPayment.amount, adPayment.currency)}</p></div><div className="rounded-xl border border-[#d9d2c4] bg-[#f7f4ed] p-4"><p className="text-[10px] uppercase text-[#7d8792]">Billing ID</p><p className="mt-2 font-mono text-sm font-extrabold">{adPayment.billingId}</p></div><div className="rounded-xl border border-[#d9d2c4] bg-[#f7f4ed] p-4"><p className="text-[10px] uppercase text-[#7d8792]">Status</p><p className="mt-2 text-sm font-extrabold">Awaiting verification</p></div></div>
          {adPayment.paymentDestination ? <div className="mt-5 rounded-xl border border-[#bfd6dc] bg-[#eef7f8] p-5"><div className="flex items-center gap-2 text-sm font-extrabold text-[#315e6c]"><Landmark className="h-4 w-4" />Provider-returned bank details</div><div className="mt-4 grid gap-3 sm:grid-cols-2 text-sm"><p><span className="text-xs text-[#477563]">Bank</span><br /><strong>{adPayment.paymentDestination.bankName}</strong></p><p><span className="text-xs text-[#477563]">Account name</span><br /><strong>{adPayment.paymentDestination.accountName}</strong></p><p className="sm:col-span-2"><span className="text-xs text-[#477563]">Account number</span><br /><strong className="font-mono text-xl">{adPayment.paymentDestination.accountNumber}</strong></p><p><span className="text-xs text-[#477563]">Exact amount</span><br /><strong className="font-mono">{money(adPayment.paymentDestination.amount, adPayment.paymentDestination.currency)}</strong></p><p><span className="text-xs text-[#477563]">Expires</span><br /><strong>{adPayment.paymentDestination.expiresAt ? new Date(adPayment.paymentDestination.expiresAt).toLocaleString() : 'Provider did not return an expiry'}</strong></p></div></div> : adPayment.paymentUrl ? <div className="mt-5 rounded-xl border border-[#d9d2c4] bg-[#f7f4ed] p-5"><p className="text-sm font-extrabold">Secure Flutterwave checkout is ready.</p><a href={adPayment.paymentUrl} className="mt-4 inline-flex items-center gap-2 rounded-xl bg-[#182333] px-4 py-2.5 text-sm font-extrabold text-[#f8f3e8]">Open checkout <ExternalLink className="h-4 w-4" /></a></div> : null}
          <p className="mt-5 text-xs leading-5 text-[#687482]">After transfer, Flutterwave verifies the transaction server-side. Do not treat a screenshot, reference-only claim, or button click as proof of payment.</p>
        </div>
      </div>}
    </div>
  </AppShell>;
