...
export default function MarketplaceManagement() {
  ...
  return <AppShell>
    <div className="mx-auto max-w-[1200px]">
      ...
      <section className="mt-8 rounded-xl border border-[#dfc27a] bg-[#fff7df] p-6"><SectionHeading eyebrow="Monthly participation subscription" title="Attach a manual payment reference" description="The $5 monthly subscription keeps approved listings visible in the general marketplace. This workspace records a reference for admin review; it does not claim card capture or external settlement." />{management.data?.billing.length ? <div className="space-y-3">{management.data.billing.map((record) => <div key={record.id} className="rounded-lg border border-[#dfc27a] bg-white/60 p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-extrabold text-[#765817]">{record.kind === 'listing' ? 'Legacy listing fee record' : 'Monthly participation fee'} · {money(record.amount, record.currency)}</p><p className="mt-1 text-xs text-[#765817]">{record.status}{record.paymentReference ? ` · ${record.paymentReference}` : ''}</p></div><Badge tone={record.status === 'paid' ? 'success' : record.status === 'submitted' ? 'info' : 'warning'}>{record.status}</Badge></div>{record.status === 'due' && record.kind === 'monthly' && <div className="mt-3 flex flex-col gap-2 sm:flex-row"><input value={paymentReference} onChange={(event) => setPaymentReference(event.target.value)} placeholder="Manual bank transfer reference" className={`${inputClass} mt-0`} /><Button className="shrink-0" onClick={() => submitReference(record.id)} disabled={paymentReference.trim().length < 3 || submitBilling.isPending}><Check className="h-4 w-4" />Submit reference</Button></div>}</div>)}</div> : <EmptyState title="No subscription records yet" description="The monthly participation record appears when a merchant submits a product for marketplace participation." />}</section>
    </div>
  </AppShell>;
}