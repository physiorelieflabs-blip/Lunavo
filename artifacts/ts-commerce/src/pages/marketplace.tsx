...
import { ArrowRight, Filter, Search, ShoppingBag, Store } from 'lucide-react';
  ...
  return <main className="min-h-[100dvh] bg-[#f5f1e8] text-[#182333]">
    ...
    <div className="mx-auto max-w-[1240px] px-5 pb-20 pt-8 md:px-10">
      <section className="relative overflow-hidden rounded-[2rem] bg-[#182333] px-6 py-12 text-[#f8f3e8] shadow-[0_20px_60px_rgba(24,35,51,.15)] md:px-12 md:py-16">
        ...
        <p className="mt-5 max-w-2xl text-sm leading-7 text-[#b8c2cc] md:text-base">A central discovery layer for merchants who choose to participate. Products are shown only when the store is active, published, and explicitly opted in.</p>
      ...
      <div className="mt-5">{products.isLoading ? <LoadingState label="Loading marketplace" /> : products.isError ? <ErrorState onRetry={() => void products.refetch()} /> : products.data?.length ? <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{products.data.map((product) => <article key={`${product.merchantKey}-${product.id}`} className="group overflow-hidden rounded-2xl border border-[#d9d2c4] bg-[#fbfaf6] shadow-[0_8px_20px_rgba(45,48,38,.035)] transition hover:-translate-y-1 hover:shadow-[0_18px_34px_rgba(45,48,38,.10)]">{product.imageUrl ? <img src={product.imageUrl} alt="" className="aspect-[4/3] w-full object-cover" /> : <div className="grid aspect-[4/3] place-items-center bg-[#e8e0cd] text-[#85601b]"><ShoppingBag className="h-8 w-8" /></div>}<div className="p-4"><div className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-[.12em] text-[#8a6826]"><Store className="h-3 w-3" />{product.merchantName}</div><h3 className="mt-2 line-clamp-2 text-base font-extrabold">{product.title}</h3>{product.category && <p className="mt-1 text-xs text-[#697687]">{product.category}</p>}<p className="mt-4 font-mono text-lg font-bold">{money(product.salePrice ?? product.price, product.currency)}</p><Link href={`/checkout/${product.merchantKey}`} className="mt-4 inline-flex items-center gap-2 text-xs font-extrabold text-[#8a6826] underline">Shop this store <ArrowRight className="h-3.5 w-3.5" /></Link></div></article>)}</div> : <EmptyState title="No marketplace products yet" description="Try a different search, or check back as participating merchants publish products." />}</div>
      <div className="mt-12 rounded-2xl border border-[#d9d2c4] bg-[#fbfaf6] p-6 text-sm text-[#697687]"><p className="font-extrabold text-[#182333]">For merchants</p><p className="mt-1 leading-6">Publish a product and enable marketplace visibility from your supplier workspace. Participation is optional and does not expose private customer, inventory, or financial data.</p><Link href="/sign-up" className="mt-4 inline-flex items-center gap-2 font-extrabold text-[#8a6826] underline">Open a merchant workspace <ArrowRight className="h-4 w-4" /></Link></div>
    </div>
  </main>;
}