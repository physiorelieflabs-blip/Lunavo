...
const PAYMENT_CURRENCIES = ['USD', 'NGN', 'GHS', 'KES', 'ZAR', 'GBP', 'EUR', 'CAD', 'AUD'] as const;
...
export default function PaymentLinkCheckout() {
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    setMessage('');
     checkout.mutate({ token, data: { customerName, customerEmail, customerPhone: customerPhone || undefined, customerCountry: customerCountry.trim() || undefined, shippingAddress, paymentCurrency: paymentCurrency || link.data.currency, marketingConsent, idempotencyKey: crypto.randomUUID() } }, {
       onSuccess: (result) => {
      ...
      <header className="mb-8 flex items-center justify-between gap-4"><p className="font-mono text-xs font-medium tracking-[.08em]">TS / COMMERCE</p><span className="flex items-center gap-2 text-xs font-bold text-[#697687]"><ShieldCheck className="h-4 w-4 text-[#2f6958]" />Secure order intake</span></header>
       <section className="rounded-2xl border border-[#d9d2c4] bg-[#fbfaf6] p-6 shadow-[0_18px_38px_rgba(31,43,56,.07)] md:p-8">
        ...
        {message && <div className="mt-5"><Notice tone={message.startsWith('This') ? 'danger' : 'success'} title={message.startsWith('This') ? 'Order not created' : 'Checkout update'}>{message}</Notice></div>}
         {!completed && <div className="mt-5 grid gap-4 rounded-xl border border-[#bfd6dc] bg-[#eef7f8] p-4 sm:grid-cols-2"><label className="block text-sm font-bold text-[#315e6c]">Pay in currency<select value={paymentCurrency || link.data.currency} onChange={(event) => setPaymentCurrency(event.target.value)} className={inputClass}>{PAYMENT_CURRENCIES.map((currency) => <option key={currency} value={currency}>{currency}{currency === link.data.currency ? ' · link currency' : ''}</option>)}</select><span className="mt-1 block text-xs font-normal">Flutterwave charges the currency you choose and TS Commerce keeps the merchant settlement currency separate.</span></label><label className="block text-sm font-bold text-[#315e6c]">Country<input required minLength={2} maxLength={80} value={customerCountry} onChange={(event) => setCustomerCountry(event.target.value)} className={inputClass} placeholder="Country you are paying from" autoComplete="country-name" /></label></div>}
