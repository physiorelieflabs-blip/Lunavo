...
export default function PublicPaymentReturn() {
  ...
  return <main className="grid min-h-[100dvh] place-items-center bg-[#f5f1e8] px-5 py-10 text-[#182333]">
    <section className="w-full max-w-[560px] rounded-2xl border border-[#d9d2c4] bg-[#fbfaf6] p-7 shadow-[0_18px_38px_rgba(31,43,56,.07)] md:p-10">
      <p className="font-mono text-[10px] uppercase tracking-[.18em] text-[#a2772e]">Whop payment return</p>
      {loading ? <div className="mt-8"><LoadingState label="Verifying your payment with Whop" /></div> : error ? <div className="mt-7"><Notice tone="danger" title="Payment verification failed">{error}</Notice><Button variant="secondary" className="mt-5" onClick={() => void verify()}><RefreshCw className="h-4 w-4" />Try verification again</Button></div> : result ? <div className="mt-7">
        ...
        {result.status !== 'paid' && <div className="mt-5 flex flex-wrap gap-3"><Button onClick={() => void verify()}><RefreshCw className="h-4 w-4" />Check again</Button><Button variant="secondary" disabled={retrying} onClick={() => void retryCheckout()}>{retrying ? 'Preparing…' : 'Retry payment'}</Button></div>}
      </div> : null}
    </section>
  </main>;
}