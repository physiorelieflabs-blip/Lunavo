...
export default function PublicStorefront() {
  ...
  return <main className="min-h-[100dvh]" style={{ backgroundColor: theme.backgroundColor, color: theme.textColor }}>
    ...
    {announcement?.body && <div className="mx-auto max-w-[1200px] px-5 pt-5 md:px-10"><div className="rounded-2xl px-5 py-4 text-sm font-bold" style={{ backgroundColor: `${theme.accentColor}18` }}>{announcement.body}</div></div>}
    <div className="mx-auto max-w-[1200px] px-5 pb-20 pt-8 md:px-10 md:pt-12">
      <section className={`relative overflow-hidden rounded-[2rem] border px-6 py-12 md:px-12 md:py-20 ${theme.layout === 'minimal' ? 'bg-white/40' : 'bg-[#182333] text-[#f8f3e8]'}`} style={theme.layout === 'minimal' ? { borderColor: `${theme.accentColor}44` } : undefined}>
      ...
      <section id="collection" className="scroll-mt-8 pt-6"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="font-mono text-[10px] uppercase tracking-[.18em]" style={{ color: theme.accentColor }}>{productsSection?.type === 'products' ? 'Collection' : 'Available now'}</p><h2 className="mt-2 text-3xl font-extrabold tracking-[-.06em]">{productsSection?.heading || 'Shop the collection'}</h2></div><p className="text-xs opacity-60">{data.products.length} published product{data.products.length === 1 ? '' : 's'}</p></div>
        {data.products.length ? <div className={`mt-7 grid gap-5 ${theme.layout === 'catalog' ? 'sm:grid-cols-3 lg:grid-cols-4' : 'sm:grid-cols-2 lg:grid-cols-3'}`}>{data.products.map((product) => { const unavailable = product.inventoryStatus === 'out_of_stock' || product.inventoryQuantity === 0; return <article key={product.id} className="overflow-hidden rounded-2xl border bg-white/65 shadow-[0_12px_30px_rgba(24,35,51,.06)]" style={{ borderColor: `${theme.accentColor}33` }}><div className="aspect-[4/3] bg-black/5">{product.imageUrl ? <img src={product.imageUrl} alt={product.title} className="h-full w-full object-cover" /> : <div className="grid h-full place-items-center opacity-50"><ShoppingBag className="h-9 w-9" /></div>}</div><div className="p-5"><div className="flex items-center justify-between gap-3">{product.category && <p className="text-[10px] font-bold uppercase tracking-[.14em] opacity-55">{product.category}</p>}{unavailable && <span className="text-[10px] font-bold uppercase tracking-[.1em] text-[#a33e38]">Sold out</span>}</div><h3 className="mt-2 text-lg font-extrabold">{product.title}</h3>{product.description && <p className="mt-2 line-clamp-3 text-xs leading-5 opacity-65">{product.description}</p>}<div className="mt-5 flex items-center justify-between gap-3"><p className="font-mono text-lg font-bold">{money(product.salePrice ?? product.price, product.currency)}</p><Link href={`/checkout/${data.merchantKey}?productId=${product.id}`} className="inline-flex items-center gap-1.5 rounded-lg px-3 py-2 text-xs font-extrabold" style={{ backgroundColor: unavailable ? '#d9d2c4' : theme.accentColor, color: unavailable ? '#697687' : theme.textColor, pointerEvents: unavailable ? 'none' : undefined }}>View & order <ArrowRight className="h-3.5 w-3.5" /></Link></div></div></article>})}</div> : <div className="mt-7"><EmptyState title="The collection is being prepared" description="This store has not published any priced products yet." /></div>}
      </section>
