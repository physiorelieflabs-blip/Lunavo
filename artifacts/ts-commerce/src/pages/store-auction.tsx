...
import { ArrowLeft, BarChart3, Gavel, ShieldCheck, Store, TrendingUp, type LucideIcon } from 'lucide-react';
...
type Metric = { revenue_minor:number|null; verified_profit_minor:number|null; orders_count:number; customers_count:number; aov_minor:number|null; growth_percent:number|null; traffic_count:number|null; conversion_percent:number|null; expenses_minor:number|null; ad_spend_minor:number|null; ad_revenue_minor:number|null; store_age_days:number|null; inventory_units:number|null; valuation_indicator_minor:number|null; currency:string; captured_at:string };
type Bid = { amount:number; currency:string; created_at:string };

function optionalMetric(value:number|null|undefined, format:(n:number)=>string){ return value == null ? '—' : format(Number(value)); }
function optionalPercent(value:number|null|undefined){ return value == null ? '—' : `${Number(value)}%`; }
...
export default function StoreAuction() {
  ...
  const headlineMetrics: Array<{ icon:LucideIcon; label:string; value:React.ReactNode }> = [
    { icon:TrendingUp, label:'Revenue', value:latest ? optionalMetric(latest.revenue_minor,n=>money(n/100,latest!.currency)) : '—' },
    { icon:BarChart3, label:'Verified profit', value:latest ? optionalMetric(latest.verified_profit_minor,n=>money(n/100,latest!.currency)) : '—' },
    { icon:Store, label:'Orders', value:latest ? String(latest.orders_count) : '—' },
    { icon:Gavel, label:'Customers', value:latest ? String(latest.customers_count) : '—' },
  ];
...
<section className="mt-7 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{headlineMetrics.map(({icon:Icon,label,value})=><div key={label} className="rounded-2xl border border-[#d9d2c4] bg-[#fbfaf6] p-5"><span className="grid h-10 w-10 place-items-center rounded-xl bg-[#e8eef3] text-[#315e6c]"><Icon className="h-5 w-5"/></span><p className="mt-4 text-xs font-bold text-[#697687]">{label}</p><p className="mt-1 font-mono text-2xl font-extrabold">{value}</p></div>)}</section>

<section className="mt-7 grid gap-7 lg:grid-cols-[1.25fr_.75fr]"><div className="rounded-2xl border border-[#d9d2c4] bg-[#fbfaf6] p-6 md:p-7"><div className="flex items-center gap-3"><BarChart3 className="h-5 w-5 text-[#c85d3f]"/><div><p className="font-mono text-[10px] uppercase tracking-[.15em] text-[#a2772e]">Verified performance</p><h2 className="font-extrabold">Revenue history</h2></div></div>{revenueHistory.length ? <><div className="mt-7 flex h-48 items-end gap-2 border-b border-[#e3ddd2]">{revenueHistory.map((point,index)=><div key={`${point.captured_at}-${index}`} className="flex min-w-0 flex-1 flex-col items-center justify-end gap-2"><div title={optionalMetric(point.revenue_minor,n=>money(n/100,point.currency))} className="w-full rounded-t-md bg-[#315e6c]" style={{height:`${Math.max(5,Math.round(Number(point.revenue_minor || 0)/maxRevenue*100))}%`}}/><span className="text-[8px] text-[#7b8795]">{new Date(point.captured_at).toLocaleDateString(undefined,{month:'short',day:'numeric'})}</span></div>)}</div></> : <p className="mt-8 rounded-xl bg-[#f5f1e8] p-5 text-sm text-[#697687]">No verified historical revenue snapshots are available yet.</p>}<div className="mt-5 grid gap-3 sm:grid-cols-3"><div><p className="text-[10px] uppercase text-[#697687]">AOV</p><p className="mt-1 font-mono font-bold">{optionalMetric(latest?.aov_minor,n=>money(n/100,latest!.currency))}</p></div><div><p className="text-[10px] uppercase text-[#697687]">Ad spend</p><p className="mt-1 font-mono font-bold">{optionalMetric(latest?.ad_spend_minor,n=>money(n/100,latest!.currency))}</p></div><div><p className="text-[10px] uppercase text-[#697687]">Inventory</p><p className="mt-1 font-mono font-bold">{latest?.inventory_units == null ? '—' : latest.inventory_units}</p></div></div></div>
...
<section className="mt-7 rounded-2xl border border-[#d9d2c4] bg-[#fbfaf6] p-6"><h2 className="font-extrabold">Verified acquisition facts</h2><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{[['Store age',latest?.store_age_days == null?'—':`${latest.store_age_days} days`],['Growth',optionalPercent(latest?.growth_percent)],['Conversion',optionalPercent(latest?.conversion_percent)],['Valuation indicator',optionalMetric(latest?.valuation_indicator_minor,n=>money(n/100,latest!.currency))]].map(([k,v])=><div key={k} className="rounded-xl bg-[#f5f1e8] p-4"><p className="text-[10px] uppercase text-[#697687]">{k}</p><p className="mt-1 font-mono font-bold">{v}</p></div>)}</div><p className="mt-5 text-xs leading-5 text-[#697687]">A dash means Lunavo has no independent verified source for that metric at this time; it is not a zero. Private customer identities, credentials and other restricted records are never exposed on the public auction page. ownership transfers only after server-side payment verification.</p></section></div></main>;
}
