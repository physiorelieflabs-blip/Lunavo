...
<section className="mt-7 grid gap-7 lg:grid-cols-[1.25fr_.75fr]"><div className="rounded-2xl border border-[#d9d2c4] bg-[#fbfaf6] p-6 md:p-7"><div className="flex items-center gap-3"><BarChart3 className="h-5 w-5 text-[#c85d3f]"/><div><p className="font-mono text-[10px] uppercase tracking-[.15em] text-[#a2772e]">Verified performance</p><h2 className="font-extrabold">Revenue history</h2></div></div>{revenueHistory.length ? <><div className="mt-7 flex h-48 items-end gap-2 border-b border-[#e3ddd2]">{revenueHistory.map((point,index)=><div key={`${point.captured_at}-${index}`} className="flex min-w-0 flex-1 flex-col items-center justify-end gap-2"><div title={money(Number(point.revenue_minor)/100,point.currency)} className="w-full rounded-t-md bg-[#315e6c]" style={{height:`${Math.max(5,Math.round(Number(point.revenue_minor)/maxRevenue*100))}%`}}/><span className="text-[8px] text-[#7b8795]">{new Date(point.captured_at).toLocaleDateString(undefined,{month:'short',day:'numeric'})}</span></div>)}</div></> : <p className="mt-8 rounded-xl bg-[#f5f1e8] p-5 text-sm text-[#697687]">No verified historical revenue snapshots are available yet.</p>}<div className="mt-5 grid gap-3 sm:grid-cols-3"><div><p className="text-[10px] uppercase text-[#697687]">AOV</p><p className="mt-1 font-mono font-bold">{latest?money(Number(latest.aov_minor)/100,latest.currency):'—'}</p></div><div><p className="text-[10px] uppercase text-[#697687]">Ad spend</p><p className="mt-1 font-mono font-bold">{optionalMetric(latest?.ad_spend_minor,n=>money(n/100,latest!.currency))}</p></div><div><p className="text-[10px] uppercase text-[#697687]">Inventory</p><p className="mt-1 font-mono font-bold">{latest?.inventory_units == null ? '—' : latest.inventory_units}</p></div></div></div>
