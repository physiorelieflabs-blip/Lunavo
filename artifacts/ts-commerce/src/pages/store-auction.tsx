...
<section className="mt-7 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{[[TrendingUp,'Revenue',latest?optionalMetric(latest.revenue_minor,n=>money(n/100,latest!.currency)):'—'],[BarChart3,'Verified profit',latest?optionalMetric(latest.verified_profit_minor,n=>money(n/100,latest!.currency)):'—'],[Store,'Orders',latest?String(latest.orders_count):'—'],[Gavel,'Customers',latest?String(latest.customers_count):'—']].map(([Icon,label,value])=><div key={String(label)} className="rounded-2xl border border-[#d9d2c4] bg-[#fbfaf6] p-5"><span className="grid h-10 w-10 place-items-center rounded-xl bg-[#e8eef3] text-[#315e6c]"><Icon className="h-5 w-5"/></span><p className="mt-4 text-xs font-bold text-[#697687]">{label}</p><p className="mt-1 font-mono text-2xl font-extrabold">{value}</p></div>)}</section>
