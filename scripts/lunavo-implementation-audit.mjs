...
const auctioneer = await read(requiredFiles[9]);
...
const auctioneerRoute = await read(requiredFiles[10]);
...
const dashboard = await read(requiredFiles[11]);
...
const migration = await read(requiredFiles[12]);
