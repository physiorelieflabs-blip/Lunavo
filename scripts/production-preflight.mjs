...
if(strict){requireEnv('DATABASE_URL');requireUrl('APP_BASE_URL');}
...
if(strict&&!process.env.FLUTTERWAVE_SECRET_KEY&&!process.env.FLW_SECRET_KEY)warnings.push('Flutterwave Secret Key is not configured; real-money checkout remains disabled until the Master Admin configures it.');
if(strict&&!process.env.FLUTTERWAVE_WEBHOOK_SECRET&&!process.env.FLW_WEBHOOK_HASH)warnings.push('Flutterwave Webhook Secret is not configured; provider webhook settlement remains disabled until configured.');
